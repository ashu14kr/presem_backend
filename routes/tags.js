...
router.get(
	...
	asyncHandler(async (req, res, next) => {
		...
		let { rows } = await pool.query(
			`select avatar_url,first_name,last_name,username from songs left join songappusers on songs.userid=songappusers.username where '${req.params.tag}'= any(tags) offset $1 limit $2;`,
			[offset, limit]
		)
